...
public class SQLiteUtil extends SQLiteOpenHelper {
    ...
    public List<Transaction> getAllTransaction() {
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT * FROM " + TABLE_TRANSACTION + " ORDER BY " + TRANSACTION_DATE + " ASC";
    ...
    public Transaction getTransactionById(int id) {
        ...
        if (c == null) {
            ...
            if (!c.isAfterLast()) {
                ...
                float amount = c.getInt(c.getColumnIndex(TRANSACTION_AMOUNT));
                String note = c.getString(c.getColumnIndex(TRANSACTION_NOTE));
                int walletType = c.getInt(c.getColumnIndex(TRANSACTION_WALLET_TYPE));
                int groupId = c.getInt(c.getColumnIndex(TRANSACTION_GROUP_ID));
                Date date = ConversionUtil.timestampToDate(c.getLong(c.getColumnIndex(TRANSACTION_DATE)));
    ...
    public List<Transaction> getTransactionInRange(Date startDate, Date endDate) {
        long startDateInMilliseconds = ConversionUtil.dateToTimestamp(startDate);
        long endDateInMilliseconds = ConversionUtil.dateToTimestamp(endDate);

        SQLiteDatabase db = this.getReadableDatabase();

        String query = "SELECT * FROM " + TABLE_TRANSACTION + " WHERE " + TRANSACTION_DATE + " >= " + startDateInMilliseconds + " AND " + TRANSACTION_DATE + " < " + endDateInMilliseconds + " ORDER BY " + TRANSACTION_DATE + " ASC";
        //System.out.println(query);
    ...
    public long getMoneyFromADateTillNow(Date date, int walletType) {
        long dateInMilliseconds = ConversionUtil.dateToTimestamp(date);
